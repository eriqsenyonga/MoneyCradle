...
public class BarGraphIncomeProject {
	Bundle dataBundle;
	float textSize;
	ConversionClass mCC;

	public BarGraphIncomeProject(Bundle bundle) {
	...
	public GraphicalView getProjectsIncomeBarView(Context context) {
		...
		CategorySeries series = new CategorySeries("Projects Total Income");
		...
		mRenderer.setChartTitle("TOTAL INCOME PER PROJECT");
	...
	public GraphicalView getProjectIncomeVsExpensesBarView(Context context) {
		...
		String[] projectNames = dataBundle.getStringArray("projectNames");
		...
		for (double totIncome : totalIncome) {
			...
			totIncomeSeries.add(projectNames[i], totIncome);
			i = i + 1;
		...
		for (double totExpense : totalExpense) {
			...
			totExpenseSeries.add(projectNames[k], totExpense);
			k = k + 1;
		...
		mRenderer.setYTitle("Amount of Money");
		mRenderer.setBarSpacing(0.2);
		mRenderer.setXAxisMax(projectNames.length + 0.8);
		mRenderer.setXAxisMin(0.2);
		mRenderer.setLabelsTextSize(textSize);
		mRenderer.setApplyBackgroundColor(true);
		mRenderer.setBackgroundColor(Color.WHITE);
		...
		for (int h = 0; h < projectNames.length; h++) {
			mRenderer.addXTextLabel(h + 1, projectNames[h]);
		}
	...
	public GraphicalView getProjectsIncomePieChartView(Context context) {

		Log.d("achart get PIE CHART", "GET PIE CHART PROJECTS INCOME");
		...
		CategorySeries series = new CategorySeries(
				"Projects Total Income Pie Chart");
