...
public class BarGraphIncomeCategories {

	Bundle dataBundle;
	float textSize;
	ConversionClass mCC;

	public BarGraphIncomeCategories(Bundle bundle) {
	...
	public GraphicalView getCategoryIncomeBarView(Context context) {
		...
		String[] x = dataBundle.getStringArray("categoryName");

		CategorySeries series = new CategorySeries("Categories Total Income");
		...
		mRenderer.setChartTitle("TOTAL INCOME PER CATEGORY");
	...
	public GraphicalView getCategoryIncomeVsExpensesBarView(Context context) {
		...
		String[] categoryNames = dataBundle.getStringArray("categoryNames");
		...
		for (double totIncome : totalIncome) {
			...
			totIncomeSeries.add(categoryNames[i], totIncome);
			i = i + 1;
		...
		for (double totExpense : totalExpense) {
			...
			totExpenseSeries.add(categoryNames[k], totExpense);
			k = k + 1;
		...
		mRenderer.setYTitle("Amount of Money");
		mRenderer.setBarSpacing(0.2);
		mRenderer.setXAxisMax(categoryNames.length + 0.8);
		mRenderer.setXAxisMin(0.2);
		mRenderer.setLabelsTextSize(textSize);
		mRenderer.setApplyBackgroundColor(true);
		mRenderer.setBackgroundColor(Color.WHITE);
		...
		for (int h = 0; h < categoryNames.length; h++) {
			mRenderer.addXTextLabel(h + 1, categoryNames[h]);
		}
	...
	public GraphicalView getCategoryIncomePieChartView(Context context) {
		...
		CategorySeries series = new CategorySeries(
				"Categories Total Income Pie Chart");
