...
public class BarGraphIncomePayee {
	Bundle dataBundle;
	float textSize;
	ConversionClass mCC;

	public BarGraphIncomePayee(Bundle bundle) {
	...
	public GraphicalView getPayeesIncomeBarView(Context context) {
		...
		String[] x = dataBundle.getStringArray("payeeName");

		CategorySeries series = new CategorySeries("Payees Total Income");
		...
		mRenderer.setChartTitle("TOTAL INCOME PER PAYEE");
	...
	public GraphicalView getPayeeIncomeVsExpensesBarView(Context context) {
		...
		String[] payeeNames = dataBundle.getStringArray("payeeNames");
		...
		for (double totIncome : totalIncome) {
			...
			totIncomeSeries.add(payeeNames[i], totIncome);
			i = i + 1;
		...
		for (double totExpense : totalExpense) {
			...
			totExpenseSeries.add(payeeNames[k], totExpense);
			k = k + 1;
		...
		mRenderer.setYTitle("Amount of Money");
		mRenderer.setBarSpacing(0.2);
		mRenderer.setXAxisMax(payeeNames.length + 0.8);
		mRenderer.setXAxisMin(0.2);
		mRenderer.setLabelsTextSize(textSize);
		mRenderer.setApplyBackgroundColor(true);
		mRenderer.setBackgroundColor(Color.WHITE);
		...
		for (int h = 0; h < payeeNames.length; h++) {
			mRenderer.addXTextLabel(h + 1, payeeNames[h]);
		}
	...
	public GraphicalView getPayeesIncomePieChartView(Context context) {

		Log.d("achart get PIE CHART", "GET PIE CHART PAYEES INCOME");
		...
		Log.d("achart get PIE CHART", "END OF GET PIE CHART PAYEE iNCOME");
